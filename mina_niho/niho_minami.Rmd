...
j <- meshcode_to_latlong(code = 564000463 , loc = "C")
grides <- (fuk_pop[,1])
head(grides[2])
grides <- as.matrix(grides)
grd <- fuk_pop$gridcode
head(grd);class(grd)
head(grides)
class(grides)
all <- apply(X = grides,MARGIN=c(1,2),FUN = meshcode_to_latlong)
head(all)
args(apply)
trial <- meshcode_to_latlong(code = grides[2])
trial
#create the lat/long
for (i in 1:length(fuk_pop$gridcode) ){
        lis <- meshcode_to_latlong(code = grides[i], loc = "C")
        dat <- data.frame(lis[i])
}
?data.frame
#test
head(lis);length(lis)
length(fuk_pop$gridcode)
class(lis);tail(lis)
head(dat);class(dat);length(dat)
```
