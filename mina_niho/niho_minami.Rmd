...
meshcode_to_latlong <- function (code, loc = "C"){
        ...
        if (length (grep ( "^ [0-9] {6}", code)) == 1) {# secondary mesh or more
        ...
        if (length (grep ( "^ [0-9] {8}", code)) == 1) {# tertiary mesh
        ...
        # Following, seek the southwest corner coordinates.
        
        lat <- code12 * 2/3; # primary mesh
        long <- code34 + 100;
        
        if (exists ( "code5") && exists ( "code6")) {# secondary mesh
                lat <- lat + (code5 * 2/3) / 8;
                long <- long + code6 / 8;
        }
        if (exists ( "code7") && exists ( "code8")) {# tertiary mesh
                lat <- lat + (code7 * 2/3) / 8/10;
                long <- long + code8 / 8/10;
        }
        
        if (loc == "C") {# the center of the coordinate
                lat <- lat + lat_width / 2;
                long <- long + long_width / 2;
        }
        if (length (grep ( "N", loc)) == 1) {# northern end of the coordinates
                lat <- lat + lat_width;
        }
        if (length (grep ( "E", loc) == 1)) {# eastern end of the coordinates
...
jio <- meshcode_to_latlong(code = fuk_pop$gridcode)
